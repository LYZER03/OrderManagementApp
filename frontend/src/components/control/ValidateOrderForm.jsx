...
const ValidateOrderForm = ({ open, onClose, order, onOrderValidated }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Appel API pour valider le contrôle
      ...
      console.error('Erreur lors de la validation du contrôle', err);
      setError(err.message || 'Une erreur est survenue lors de la validation du contrôle');
      setSwipeProgress(0);
      setIsSwipeComplete(false);
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      <DialogTitle sx={{ 
        ...
        bgcolor: success ? theme.palette.success.light : theme.palette.background.paper,
        color: success ? theme.palette.success.contrastText : 'inherit',
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <AnimatePresence>
            {success ? (
              ...
              >
                <CheckCircleIcon sx={{ color: theme.palette.success.contrastText }} />
              </motion.div>
            ) : null}
          </AnimatePresence>
          {success ? 'Contrôle validé !' : 'Valider le contrôle'}
        </Box>
        {!loading && (
          <IconButton
            onClick={handleClose}
      ...
      <DialogContent sx={{ px: isMobile ? 2 : 3 }}>
        <DialogContentText sx={{ mb: 2 }}>
          Confirmez la validation du contrôle de cette commande.
        </DialogContentText>
        ...
        <Box sx={{ mb: 3, mt: 1 }}>
          ...
          <Typography variant="subtitle2" gutterBottom>
            Nombre de lignes: <Chip label={order.line_count || 'Non défini'} color={order.line_count ? 'success' : 'default'} size="small" />
          </Typography>
          <Typography variant="subtitle2" gutterBottom>
            Préparée le: <strong>
              {order.prepared_at ? 
          ...
          <Typography variant="subtitle2" gutterBottom>
            Préparée par: <strong>{order.preparer ? order.preparer.username : 'Non spécifié'}</strong>
          </Typography>
        </Box>
        ...
        {!success && (
          <>
            ...
            >
              ...
              >
                {loading ? 'Validation en cours...' : 'Glissez pour valider le contrôle'}
              </Box>
            </Box>
